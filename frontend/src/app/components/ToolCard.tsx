  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: "#1B3022",
          border: `1px solid ${hovered ? "rgba(164,251,204,0.30)" : "rgba(164,251,204,0.10)"}`,
          transition:
            "transform 0.3s cubic-bezier(0.22,1,0.36,1), border-color 0.3s ease, box-shadow 0.3s ease",
          transform: hovered ? "translateY(-6px)" : "translateY(0)",
          animationDelay: `${index * 80}ms`,
          animation: "toolCardReveal 0.55s ease forwards",
          opacity: 0,
          boxShadow: hovered
            ? "0 24px 48px -8px rgba(0,0,0,0.65), 0 0 0 1px rgba(164,251,204,0.15), inset 0 0 40px rgba(164,251,204,0.03)"
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            background: `radial-gradient(200px circle at ${mousePos.x}% ${mousePos.y}%, rgba(164,251,204,0.09) 0%, transparent 70%)`,
          }}
        />
        ...
        <div className="relative p-6 flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-1.5">
              {tool.badge && (
                <span
                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-full tracking-wide ${BADGE_STYLES[tool.badge]}`}
                >
                  {tool.badge}
                </span>
              )}
              {tool.stats && (
                <span className="text-[11px]" style={{ color: "#9199A5" }}>
                  {tool.stats}
                </span>
              )}
            </div>
          ...
          {/* CTA */}
          <div
            className="flex items-center gap-2 text-xs font-semibold mt-auto pt-2"
            style={{
              color: "#A4FBCC",
          ...
          <div className="mt-6">
            {tool.website_url.startsWith("/") ? (
              <Link
                ...
                onClick={() => setModalOpen(false)}
                onMouseEnter={(e) =>
                  ((e.currentTarget as HTMLElement).style.background = "rgba(164,251,204,0.18)")
                }
                onMouseLeave={(e) =>
                  ((e.currentTarget as HTMLElement).style.background = "rgba(164,251,204,0.10)")
                }
              >
                Launch Tool
                <svg
