  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: "#0F2C1A",
          border: `1px solid ${hovered ? "rgba(164,251,204,0.45)" : "rgba(164,251,204,0.15)"}`,
          transition:
            "transform 0.35s cubic-bezier(0.22,1,0.36,1), box-shadow 0.35s ease, border-color 0.3s ease",
          transform: hovered ? "translateY(-6px)" : "translateY(0)",
          boxShadow: hovered
            ? "0 24px 56px -8px rgba(0,0,0,0.5), 0 0 0 1px rgba(164,251,204,0.12)"
            : "0 2px 8px rgba(0,0,0,0.2)",
          animation: "projectCardReveal 0.55s ease forwards",
        }}
      >
        ...
        <div
          ...
          style={{
            opacity: hovered ? 1 : 0,
            transition: "opacity 0.3s ease",
            background: `radial-gradient(260px circle at ${mousePos.x}% ${mousePos.y}%, rgba(164,251,204,0.12) 0%, transparent 70%)`,
          }}
        />
        ...
        <div className="relative overflow-hidden flex-shrink-0" style={{ height: imageHeight }}>
          ...
          <div
            className="absolute inset-x-0 bottom-0 h-20"
            style={{ background: "linear-gradient(to top, #0F2C1A 0%, transparent 100%)" }}
          />
          <div
            className="absolute top-3 right-3 text-xs z-10"
            style={{
        ...
        <div className="relative flex flex-col gap-3 p-5 flex-1">
          ...
          <p className="text-sm leading-relaxed flex-1" style={{ color: "#B0C4B0" }}>
            {project.tagline}
          </p>

          <div className="flex flex-wrap gap-1.5">
            {project.tags.slice(0, 4).map((tag: string) => (
              <span key={tag} className="text-xs" style={{ color: "#B0C4B0", background: "rgba(164,251,204,0.1)", padding: "2px 8px", borderRadius: "6px" }}>
                {tag}
              </span>
            ))}
            {project.tags.length > 4 && (
              <span className="text-xs self-center" style={{ color: "#B0C4B0", fontFamily: "'Space Mono', monospace" }}>
                +{project.tags.length - 4}
              </span>
            )}
          </div>

          <div className="flex items-center justify-between pt-3" style={{ borderTop: "1px solid rgba(164,251,204,0.1)" }}>
            {onClick ? (
              <button
                className="text-sm flex items-center gap-1.5"
                style={{
              ...
              >
                Case Study
                <ArrowUpRight size={13} style={{ transition: "transform 0.2s ease", transform: hovered ? "translate(2px, -2px)" : "translate(0, 0)" }} />
              </button>
            ) : (
              <Link
                href={`/projects/${project.slug}`}
                className="text-sm flex items-center gap-1.5"
                style={{ color: "#A4FBCC", textDecoration: "none", transition: "gap 0.2s ease" }}
              >
                Case Study
                <ArrowUpRight size={13} style={{ transition: "transform 0.2s ease", transform: hovered ? "translate(2px, -2px)" : "translate(0, 0)" }} />
              </Link>
            ...
            <div className="flex items-center gap-3">
              {project.github && (
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="transition-colors duration-200 text-[#B0C4B0] hover:text-white">
                  <Github size={15} />
                </a>
              )}
              {project.demo && (
                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="transition-colors duration-200 text-[#B0C4B0] hover:text-white">
                  <ExternalLink size={15} />
                </a>
              )}
            </div>
