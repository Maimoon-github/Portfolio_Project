...
export const TerminalPanel: React.FC<TerminalPanelProps> = ({
  ...
  return (
    <motion.div
      ...
      role="region"
      aria-label="Terminal simulation"
      className={`rounded-2xl overflow-hidden shadow-2xl ring-1 ring-accent/30 backdrop-blur-sm ${className}`}
    >
      <div className="flex flex-col md:flex-row">
        <div className="flex-1 bg-gradient-to-br from-surface-container to-surface-container-low p-4 md:p-6">
          <div className="flex items-center gap-3 mb-3">
            <span className="h-3 w-3 rounded-full bg-red-500/90 shadow-sm" aria-hidden />
            <span className="h-3 w-3 rounded-full bg-yellow-400/90 shadow-sm" aria-hidden />
            <span className="h-3 w-3 rounded-full bg-accent/80 shadow-sm" aria-hidden />
            <div className="ml-auto text-sm text-accent/80">portfolio.shell</div>
          </div>

          <div className="font-mono text-sm leading-relaxed text-on-background min-h-[96px]">
            <span className="text-accent">$</span> <span aria-live="polite">{text}</span>
            <span className="inline-block w-3 h-[18px] bg-accent/70 align-middle ml-1" />
          </div>
        </div>

        <div className="w-full md:w-56 bg-surface-container-lowest/60 p-3 md:p-4 flex flex-col gap-2">
          <div className="flex items-center justify-between text-xs text-accent/80">
            <div className="uppercase tracking-wide text-[10px]">controls</div>
            <div className="text-[11px]">speed</div>
          </div>

          <div className="flex items-center gap-2">
            <button onClick={() => (isPlaying ? pause() : play())} aria-pressed={isPlaying} className="flex-1 px-3 py-2 rounded-md glass text-sm">
              {isPlaying ? "Pause" : "Play"}
            </button>
            <button onClick={reset} className="px-3 py-2 rounded-md glass text-sm">Reset</button>
          </div>

          <div className="flex items-center gap-2 mt-1">
            <button onClick={handleCopy} className="flex-1 px-3 py-2 rounded-md glass text-sm">Copy</button>
            <a href="#projects" className="px-3 py-2 rounded-md bg-accent/90 hover:bg-accent text-on-accent text-sm font-medium transition text-center">Explore</a>
          </div>

          <div className="mt-auto text-[12px] text-accent/60">Tip: Press Play to animate.</div>
        </div>
      </div>
    </motion.div>
  );
