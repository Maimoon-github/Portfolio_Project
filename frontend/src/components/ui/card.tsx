...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-6 rounded-xl border border-[var(--color-glass-border)] bg-[var(--color-surface-container-low)] text-[var(--color-on-background)]",
        className,
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <p data-slot="card-description" className={cn("text-sm text-[var(--color-outline)]", className)} {...props} />
  );
}
